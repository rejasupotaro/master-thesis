...
def generate(train_size=0.8):
    ...
    get_logger().info('Genereate listwise dataset')
    ...
    get_logger().info(f'Listwise dataset was created with {len(dataset)} groups')

    train_dataset, test_dataset = train_test_split(dataset, train_size=train_size, shuffle=True)
    with open(os.path.join(project_dir, 'data', 'processed', f'listwise.train.pkl'), 'wb') as file:
        pickle.dump(train_dataset, file)
    with open(os.path.join(project_dir, 'data', 'processed', f'listwise.test.pkl'), 'wb') as file:
