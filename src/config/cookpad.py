...
def naive_config(dataset_id: str, epochs: int, data_processor: DataProcessor) -> Tuple[TrainConfig, EvalConfig]:
    if not data_processor:
        data_processor = ConcatDataProcessor()
    train_config = TrainConfig(
        dataset_id=dataset_id,
...
def nrmf_simple_all_config(dataset_id: str, epochs: int, data_processor) -> Tuple[TrainConfig, EvalConfig]:
    if not data_processor:
        data_processor = ConcatDataProcessor()
    train_config = TrainConfig(
        dataset_id=dataset_id,
...
def nrmf_simple_query_with_1st_config(dataset_id: str, epochs: int, data_processor) -> Tuple[TrainConfig, EvalConfig]:
    if not data_processor:
        data_processor = ConcatDataProcessor()
    train_config = TrainConfig(
        dataset_id=dataset_id,
...
def fwfm_query_config(dataset_id: str, epochs: int, data_processor: DataProcessor) -> Tuple[TrainConfig, EvalConfig]:
    if not data_processor:
        data_processor = ConcatDataProcessor()
    train_config = TrainConfig(
        dataset_id=dataset_id,
...
def fwfm_all_config(dataset_id: str, epochs: int, data_processor: DataProcessor) -> Tuple[TrainConfig, EvalConfig]:
    if not data_processor:
        data_processor = ConcatDataProcessor()
    train_config = TrainConfig(
        dataset_id=dataset_id,
...
def fwfm_selected_config(dataset_id: str, epochs: int, data_processor: DataProcessor) -> Tuple[TrainConfig, EvalConfig]:
    if not data_processor:
        data_processor = ConcatDataProcessor()
    train_config = TrainConfig(
        dataset_id=dataset_id,
...
def fwfm_all_without_1st_config(dataset_id: str, epochs: int, data_processor: DataProcessor) -> Tuple[
    TrainConfig, EvalConfig]:
    if not data_processor:
        data_processor = ConcatDataProcessor()
    train_config = TrainConfig(
